...
class PowerRuleWithRadicalsPowerOfRoot(Scene):
    def construct(self):
        # Create Tex objects
        first_equality = Text('The numerator and denominator')
        first_equality1 = Text('of the power of a number')
        second_equality = Text("=")
        third_equality = Text("The denominator root of the")
        third_equality1 = Text("numerator power of that number")
        equation = Tex(
            r'$x^{\dfrac{m}{n}}=\sqrt[\leftroot{-2}\uproot{2}n]{x^m}$').scale(3)
...
#  manim -pql power_rule_with_radicals_power_of_root.py PowerRuleWithRadicalsPowerOfRoot
